...
VV.improvements = {
    ...
    load() {
        ...
        const completadas = neighborhoodImprovements.filter(i => i.status === 'Completado').length;
        const pendientes = neighborhoodImprovements.filter(i => i.status === 'Pendiente').length;
        const enProceso = neighborhoodImprovements.filter(i => i.status === 'En Proceso').length;
        const conFotos = neighborhoodImprovements.filter(i => i.photoUrl || i.completedPhotoUrl).length;
            ...
            <div class="improvement-card">
                ...
                ${imp.photoUrl ? `
                    <div style="margin: 0.5rem 0;">
                        <img src="${imp.photoUrl}" onclick="VV.improvements.viewPhoto('${imp.photoUrl}')" style="width: 100%; max-height: 200px; object-fit: cover; border-radius: 8px; cursor: pointer;" alt="Foto del problema">
                        <p style="font-size: 0.75rem; color: var(--gray-600); margin-top: 0.25rem;"><i class="fas fa-camera"></i> Problema documentado</p>
                    </div>
                ` : ''}
                ${imp.completedPhotoUrl ? `
                    <div style="margin: 0.5rem 0;">
                        <img src="${imp.completedPhotoUrl}" onclick="VV.improvements.viewPhoto('${imp.completedPhotoUrl}')" style="width: 100%; max-height: 200px; object-fit: cover; border-radius: 8px; cursor: pointer; border: 2px solid var(--success-green);" alt="Foto de mejora realizada">
    ...
    loadGallery(improvements) {
        ...
        improvements.forEach(imp => {
            ...
            if (imp.completedPhotoUrl) {
                allPhotos.push({
                    url: imp.completedPhotoUrl,
                    title: imp.title,
                    type: 'Realizada',
                    date: imp.completedAt,
                    status: 'Completado'
                });
            }
        });
